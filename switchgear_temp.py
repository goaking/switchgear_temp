...
st.write('1.左侧栏选择开关柜额定电流、目前运行电流、测温点位置及相应的温升值')
st.write('2.点击开始，根据图中当前温度☆所在位置，即可判断测量点温升状态。')
...
if st.button('开始'):
    ...
    plt.scatter(700, temp, marker='*', color='r', label='Current position中文', linewidth=1)
    plt.xlabel('min')
    plt.ylabel('K')
    plt.legend(loc=2)
    st.pyplot()
